...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    const fetchRentListings = async () =>{
      try{
        const res = await fetch(`${import.meta.env.VITE_API_KEY_ONRENDER}/api/listing/get?type=rent&limit=4`);
        const data = await res.json();
    ...
    const fetchSaleListings = async () =>{
      try{
        const res = await fetch(`${import.meta.env.VITE_API_KEY_ONRENDER}/api/listing/get?type=sale&limit=4`);
        const data = await res.json();
