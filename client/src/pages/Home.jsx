...
export default function Home() {
  ...
  useEffect(() => {
    const fetchOfferListings = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_KEY_ONRENDER}/api/listing/get?offer=true&limit=4`,
          { credentials: 'include' }
        );
    ...
    const fetchRentListings = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_KEY_ONRENDER}/api/listing/get?type=rent&limit=4`,
          { credentials: 'include' }
        );
        const data = await res.json();
    ...
    const fetchSaleListings = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_KEY_ONRENDER}/api/listing/get?type=sale&limit=4`,
          { credentials: 'include' }
        );
        const data = await res.json();
        setSaleListings(data);
        // ✅ Conteúdo pronto para exibição
        setLoading(false);
      } catch (error) {
        console.log(error);
      }
  ...
  // ⏳ Mostrar loading enquanto os dados estão a carregar
  if (loading) {
    return (
  ...
  return (
    <div>
      ...
      <Swiper
        loop={true}
        autoplay={{
          delay: offerListings.length > 0 ? 9000 / offerListings.length : 9000,
